...
from hit_omniverse.extension.hit_humanoid import HIT_HUMANOID_CFG
...
@configclass
class HitRoughPPORunnerCfg(RslRlOnPolicyRunnerCfg):
...
class BaseSceneCfg(InteractiveSceneCfg):
    ...
    robot = HIT_HUMANOID_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
    # robot = H1_2DoF_CFG
    # sensors
    ...
    height_scanner.prim_path = "{ENV_REGEX_NS}/Robot/body"
    # camera
    lidar_sensor = None
...
class EventCfg:
    ...
    add_base_mass = EventTerm(
        ...
        mode="startup",
        params={"asset_cfg": SceneEntityCfg("robot", body_names="body"), "mass_distribution_params": (-5.0, 5.0), "operation": "add"},
    )
...
@configclass
class HitBaseRoughEnvCfg(ManagerBasedRLEnvCfg):
...
@configclass
class HitBaseRoughEnvCfg_PLAY(HitBaseRoughEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
